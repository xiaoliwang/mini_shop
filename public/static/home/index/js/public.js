		...
		$('#search_key').on({
			keyup: function(e){
				...
				if(!( code ===38 || code ===40 || code ===13)){
					$('#recommend_lib').hide();
					$.ajax({
						url:'/home/entry/search_ajax?sword=' + utf8iptVal,
						type:'GET',
						dataType:'json',
						jsonp:'callback',
						success:function(data){
							...
							if( !!num ){								
								for(var i = 0; i< num; i++){
									str += '<li><div class="search-item" style="overflow: hidden;white-space: nowrap;text-overflow: ellipsis">' + data[i].key + '</div></li>';
								}								
